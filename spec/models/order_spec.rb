...
RSpec.describe Order do
  ...
  describe 'instance methods' do
    before :each do
      ...
      @order_1 = create(:order, user_id: @user.id, address_id: @user.addresses.first.id, status: "packaged")
      @order_2 = create(:order, user_id: @user.id, address_id: @user.addresses.first.id, status: "pending")
  ...
  describe 'class methods' do
    before :each do
      @user = create(:user, :with_addresses, name: 'Megan')
      @order_1 = create(:order, user_id: @user.id, status: 1)
      @order_2 = create(:order, user_id: @user.id, status: 0)
      @order_3 = create(:order, user_id: @user.id, status: 3)
