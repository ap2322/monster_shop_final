...
RSpec.describe 'User Show Page' do
  describe 'As an Admin' do
    before :each do
      ...
      @address_du = @d_user.addresses.create!(address: '123 Main St', city: 'Denver', state: 'CO', zip: 80218)
      @admin = User.create(name: 'Sal', email: 'sal@example.com', password: 'securepassword', role: 'admin')
      @address_admin = @admin.addresses.create!(address: '123 Main St', city: 'Denver', state: 'CO', zip: 80218)
    ...
    it 'I see all info a user sees, without edit ability' do
      ...
      expect(page).to have_content(@address_du.address)
      expect(page).to have_content("#{@address_du.city} #{@address_du.state} #{@address_du.zip}")
